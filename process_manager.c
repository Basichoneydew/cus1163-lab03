...
int run_basic_demo(void) {
  ...
  if (waitpid(producer_pid, &status, 0) == producer_pid) {
    printf("Producer child (PID: %d) exited with status %d\n", producer_pid,
           WEXITSTATUS(status));
  }

  if (waitpid(consumer_pid, &status, 0) == consumer_pid) {
...
void producer_process(int write_fd, int start_num) {
  ...
  // Send 5 numbers: start_num, start_num+1, start_num+2, start_num+3,
  // start_num+4
